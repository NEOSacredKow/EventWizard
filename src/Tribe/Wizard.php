...
class Wizard {
	...
	public function get_wizard_steps() {
		...
		$steps[] = $this->get_wizard_step_four(); // Add Step 4
	 ...
	 * @return array $step The array containing the step data.
	 */
/* - Original Step 3 commented out to pull defined venues instead
...
*/
	...
	// BEGIN SK 20241122 - Add venue selector to wizard
	private function get_wizard_step_three() {
	...
	//	] );
		...
		// Sort venues alphabetically by title
		...
		// Define the $step array (make sure $fields is included here)
		$step = [
			'id'          => 'event-venue',
			'step_title'  => esc_html__( 'Where', PLUGIN_TEXT_DOMAIN ),
			'title'       => esc_html__( 'Event Venue', PLUGIN_TEXT_DOMAIN ),
			'description' => esc_html__( 'Select a venue for the event.', PLUGIN_TEXT_DOMAIN ),
			'fields'      => $fields,
			'classes'     => '',
		];
	...
	public function save_selected_venues( $event_id, $_post ) {
			if ( empty( $event_id ) || empty( $_post['venues'] ) ) {
				return;
			}

			$selected_venues = array_map( 'intval', $_post['venues'] );

			// Update meta fields
			update_post_meta( $event_id, '_tribe_selected_venues', $selected_venues );

			foreach ( $selected_venues as $venue_id ) {
				add_post_meta( $event_id, '_EventVenueID', $venue_id, false );
			}
		}
	 ...
	 */
	 ...
	 private function get_wizard_step_four() {
		$fields = [];
	
		// Get the virtual event settings
		$virtual_event_settings = tribe( 'events-virtual.admin.template' )->template( 'virtual-metabox/api/inline-edit', [], false );
	
		// Add the virtual event settings as an HTML field
		$fields[] = [
			'type'    => 'html',
			'content' => $virtual_event_settings,
		];
	
		$step = [
			'id'          => 'event-virtual',
			'step_title'  => esc_html__( 'Virtual', PLUGIN_TEXT_DOMAIN ),
			'title'       => esc_html__( 'Is this a Virtual Event?', PLUGIN_TEXT_DOMAIN ),
			'description' => esc_html__( 'Configure this event as a virtual event.', PLUGIN_TEXT_DOMAIN ),
			'fields'      => $fields,
			'classes'     => '',
		];
